...
class Calibration final {
 public:
  ...
  std::filesystem::path
      save_path_;  // path to save calibrated cameras parameter
  std::filesystem::path
  ...
  std::map<std::pair<int, int>, std::vector<cv::Mat>>
      object_pose_pairs_;  // key: (objectind1,objectind2) value: Vector of
                           // poses
  std::map<std::pair<int, int>, cv::Mat>
  ...
  void saveDetectedKeypoints()
      const;  // save detection keypoints, can be
  ...
  void insertNewBoard(
      ...
      const std::filesystem::path &frame_path);  // insert a new board in all
                                                 // the different datastructure
  void insertNewObjectObservation(
      const std::shared_ptr<Object3DObs>
          new_obj_obs);                // insert new object observation
  void initializeCalibrationAllCam();  // initialize the calibration of all the
                                       // cameras
  void estimatePoseAllBoards();  // Estimate the pose of all visible boards
                                 // using a PnP
  ...
  cv::Mat computeDistanceBetweenPoints(
